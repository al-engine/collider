...
export function calcPositionFor<T extends GameObjectParams>({
  ...
  if (movingObj.speed.x === 0) {
    ...
    if (movingObj.speed.y > 0) {
      if (obstaclePosition.x < oldPosition.x + movingObj.size.width) {
        movingObj.setAbsolute({
          y: calcLimitedMoving(
            movingObj.speed.y,
            objPosition.y,
      ...
      } else if (obstaclePosition.y < oldPosition.y + movingObj.size.height) {
        movingObj.setAbsolute({
          x: calcLimitedMoving(
            movingObj.speed.x,
            objPosition.x,
      ...
      } else if (obstaclePosition.y + obstacles.size.height > oldPosition.y) {
        movingObj.setAbsolute({
          x: calcLimitedMoving(
            movingObj.speed.x,
            objPosition.x,
    ...
    if (movingObj.speed.y < 0) {
      if (obstaclePosition.x + obstacles.size.width > oldPosition.x) {
        movingObj.setAbsolute({
          y: calcLimitedMoving(
            movingObj.speed.y,
            objPosition.y,
